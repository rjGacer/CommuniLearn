...
function formatDueDisplay(due) {
  ...
  try {
    ...
    // treat as date-only when time is exactly midnight local or midnight UTC
    const isLocalMidnight = d.getHours() === 0 && d.getMinutes() === 0 && d.getSeconds() === 0;
    const isUtcMidnight = d.getUTCHours && d.getUTCHours() === 0 && d.getUTCMinutes && d.getUTCMinutes() === 0 && d.getUTCSeconds && d.getUTCSeconds() === 0;
    const isDateOnly = isLocalMidnight || isUtcMidnight;
    // if date-only, treat the intended time as end-of-day 11:59:59.999 so the due remains on that day
    if (isDateOnly) {
      // set to local end-of-day for that calendar date
      d.setHours(23, 59, 59, 999);
    }
    if (d.getTime() - now <= 0) return "Time's Up";
    if (isDateOnly) {
      return `Due on ${d.toLocaleDateString()} ${d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}`;
    }
    const mins = Math.floor(diff / 60000);
    if (mins < 60) return `Due in ${mins}m`;
    const hours = Math.round(mins / 60);
    if (hours < 24) return `Due in ${hours}h`;
    return `Due on ${d.toLocaleDateString()}`;
  } catch (e) {
    return null;
  }
}
function formatDueParts(due) {
  ...
  try {
    ...
    // if date-only, treat as end-of-day 11:59:59.999 so relative calculations and display reflect the full day
    if (isDateOnly) d.setHours(23, 59, 59, 999);
    const now = Date.now();
    const diff = d.getTime() - now;
    if (diff <= 0) return { status: 'timesUp', label: "Time's Up" };
    if (isTimeOnly) {
      return { status: 'timeOnly', timePart: d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) };
    }
    if (!isDateOnly) {
      const mins = Math.floor(diff / 60000);
      if (mins < 60) return { status: 'relative', label: `Due in ${mins}mins`, datePart: d.toLocaleDateString(), timePart: d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) };
      const hours = Math.round(mins / 60);
      if (hours < 24) return { status: 'relative', label: `Due in ${hours}hrs`, datePart: d.toLocaleDateString(), timePart: d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) };
      // more than a day, show date + time
      return { status: 'date', datePart: d.toLocaleDateString(), timePart: d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) };
    }
    // date-only: already adjusted to end-of-day above
    return { status: 'date', datePart: d.toLocaleDateString(), timePart: d.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) };
  } catch (e) {
    return null;
  }
...
export default function TeacherAnnouncements() {
  ...
  const [filterMode, setFilterMode] = useState('all'); // 'announcement' | 'attendance' | 'all'
  ...
  useEffect(() => {
    const onDocClick = e => {
      ...
      if (!target) return;
      // if clicked inside any known dropdown or its menu button, keep open
      // include both the standardized module menu classes and the announcement-specific classes
      if (target.closest('.module-dropdown') || target.closest('.module-more-btn') || target.closest('.announcement-menu') || target.closest('.announcement-menu-btn')) return;
      setOpenAnnMenu(null);
  ...
  const [editAnnAttendanceId, setEditAnnAttendanceId] = useState(null);
  ...
  const openEditAnnouncement = ann => {
    setEditAnnId(ann.id);
    // strip attendance/due markup from description when editing
    try {
      const { text, attendanceId } = parseAttendanceFromDesc(ann.description);
      setEditAnnText(text || "");
      setEditAnnAttendanceId(attendanceId);
    } catch (e) {
  ...
  const submitEditAnnouncement = async () => {
    if (!editAnnId) return;
    try {
      // re-append attendance tag if this announcement is linked to an attendance
      let description = (editAnnText || "").trim();
      if (editAnnAttendanceId) {
        // ensure no duplicate tag
  ...
  const parseAttendanceFromDesc = desc => {
    ...
    if (m) {
      ...
      // remove any appended " — Due: ..." that we add when linking attendance to announcement
      text = text.replace(/\s*—\s*Due:\s*[^\[]*/,'').trim();
      return { text, attendanceId: id };
    }
    return { text: desc, attendanceId: null };
  };
  // present modal state (teacher view)
  const [presentModalOpen, setPresentModalOpen] = useState(false);
  const [presentForAttendance, setPresentForAttendance] = useState(null);
  const [presentLoading, setPresentLoading] = useState(false);
  const [presentList, setPresentList] = useState([]);

  const showPresentList = async (attendanceId) => {
    ...
    try {
      ...
      if (!attRes.ok) {
        ...
        // still show marks if students list fails
        setPresentList(Array.isArray(attData.marks) ? attData.marks.map(m => ({ studentEmail: m.studentEmail, studentName: m.studentName || m.studentEmail, present: true, markedAt: m.createdAt })) : []);
      } else {
        const marks = Array.isArray(attData.marks) ? attData.marks : [];
        const students = Array.isArray(studentsData) ? studentsData : [];
        const marksByEmail = {};
        for (const m of marks) marksByEmail[m.studentEmail] = m;
        const combined = students.map(s => {
          const m = marksByEmail[s.email];
          return { id: s.id, studentName: s.name || s.email, studentEmail: s.email, present: !!m, markedAt: m ? m.createdAt : null };
        });
        // include any marks by emails not in students list (external/guest)
        for (const m of marks) {
          if (!students.find(s => s.email === m.studentEmail)) {
            combined.push({ id: null, studentName: m.studentName || m.studentEmail, studentEmail: m.studentEmail, present: true, markedAt: m.createdAt });
          }
        }
        setPresentList(combined);
      }
    } catch (err) {
      console.error('Failed to fetch present list', err);
  ...
  // Render Announcements (Attendances are embedded into announcements)
  // compute visible announcements based on selected filter
  ...
  return (
    <div className="announcement-wrap">
      ...
      {!loading && visibleAnnouncements.length === 0 && <p>No items match this filter.</p>}

      {!loading && visibleAnnouncements.map(a => {
        const comments = Array.isArray(a.comments) ? a.comments : [];
        const commentsNewestFirst = comments.slice().sort((x, y) => new Date(x.createdAt).getTime() - new Date(y.createdAt).getTime());
        return (
          <div className="announcement-card" key={a.id}>
            <div className="announcement-menu-btn" onClick={() => setOpenAnnMenu(prev => prev === a.id ? null : a.id)} aria-label="announcement menu" title="Actions">⋮</div>
            {openAnnMenu === a.id && (
              <div className="announcement-menu">
                {(a.teacherEmail === currentEmail || user?.role === 'teacher' || user?.role === 'superteacher') && (
                  <>
                    <p onClick={() => openEditAnnouncement(a)}>✏️ Edit</p>
                    <p onClick={() => deleteAnnouncement(a.id)}>🗑️ Delete</p>
                  </>
                )}
              </div>
            )}
            ...
            <div className="announcement-header">
              ...
              <div className="announcement-meta">
                ...
                <p>{timeAgo(new Date(a.createdAt))} - {formatDateTime(new Date(a.createdAt))}</p>
              </div>
            </div>
                  ...
                  <>
                      ...
                      <div style={{ marginTop: 8 }}>
                        {(() => {
                          ...
                          if (!markedAttendanceIds[attendanceId]) {
                            if (pastDue) return <button className="attendance-btn" disabled style={{ background: '#ef4444' }}>Time's Up</button>;
                    ...
                    {attendanceId && user?.role === 'teacher' && (
                      <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                        <button className="attendance-btn" style={{ background: '#0b74c9' }} onClick={() => showPresentList(attendanceId)}>SHOW PRESENT</button>
                        {(() => {
                          const att = attendancesList.find(x => x.id === attendanceId);
                          if (att && att.dueDate) {
                            ...
                            if (parts.status === 'relative') return <div style={{ fontSize: '0.9rem', color: '#374151' }}>{`(${parts.label}) ${parts.datePart}, ${parts.timePart}`}</div>;
      ...
      {/* Attendances are shown inline within announcements when applicable. */}

      {presentModalOpen && (
        <div className="modal-overlay" onClick={() => setPresentModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <h2 className="modal-title">Present List</h2>
            <div className="modal-form">
              {presentLoading ? (
                <p>Loading...</p>
              ) : presentList.length === 0 ? (
                <p>No students have marked present.</p>
              ) : (
                <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                  {presentList.map(p => (
                    <li key={(p.id || p.studentEmail) + (p.present ? '-present' : '-absent')} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #eee' }}>
                      <div>
                        <strong>{p.studentName}</strong>
                        <div style={{ fontSize: '0.85rem', color: '#666' }}>{p.studentEmail}</div>
                      </div>
                      <div>
                        {p.present ? (
                          <span style={{ background: '#16a34a', color: '#fff', padding: '6px 10px', borderRadius: 6, fontWeight: 600 }}>PRESENT</span>
                        ) : (
                          <span style={{ background: '#e5e7eb', color: '#374151', padding: '6px 10px', borderRadius: 6 }}>ABSENT</span>
                        )}
                      </div>
                    </li>
                  ))}
            ...
            <div className="modal-actions">
              <button className="modal-btn create" onClick={() => setPresentModalOpen(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Attendance edit modal removed; attendance actions are embedded in announcements */}
    </div>
  );
}